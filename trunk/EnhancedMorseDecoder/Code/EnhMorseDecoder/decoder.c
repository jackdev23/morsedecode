...
//TODO: Check the implications of this frequency on the functions ??
//#define F_CPU 40000000   // (Hz) Frequency of the Processor
#define F_CPU 80000000   // (Hz) Frequency of the Processor  
...
void decoder_init(uint8_t uplink_wpm, uint8_t downlink_wpm ) //sww removed  /*, PORT_t *downlink_port, uint8_t downlink_pin)*/
{
  ...
  // TODO: setup dspic ADC pin
	//PORTA.DIRSET |= PIN7_bm;

  //sww. don't think we will need this.
	//encoder_init(downlink_wpm, downlink_port, downlink_pin);

//TODO:  Need to redo the ADC init funtion for the DSPIC
...
//TODO: create similar ADCA struct to use throughout the code
//	SUPPRESS_ADC(&ADCA){
...
/*  TODO: Re-Write the ACD ISR for the DSPIC
...
void decoder_process_rx(){
	...
	while(hf.rx.read != local_wrt_index){
        ...
        //TODO: Handling of Ack and ILL will be processed in the EN_HF_PROVIDER
...
static char decoder_decode_symbols(char *symbols, uint8_t len)
{
	...
	case 2:
		...
		break;
	case 3:
		if(strncmp(symbols, "...", 3) == 0){
			return 's';
		}else if(strncmp(symbols, "..-", 3) == 0){
			return 'u';
		}else if(strncmp(symbols, ".-.", 3) == 0){
			return 'r';
		}else if(strncmp(symbols, ".--", 3) == 0){
		...
		break;
	case 4:
		if(strncmp(symbols, "....", 4) == 0){
			return 'h';
		}else if(strncmp(symbols, "...-", 4) == 0){
			return 'v';
		}else if(strncmp(symbols, "..-.", 4) == 0){
			return 'f';
		}else if(strncmp(symbols, "-...", 4) == 0){
		...
		break;
	case 5:
		if(strncmp(symbols, ".----", 5) == 0){
			return '1';
		}else if(strncmp(symbols, "..---", 5) == 0){
			return '2';
		}else if(strncmp(symbols, "...--", 5) == 0){
			return '3';
		}else if(strncmp(symbols, "....-", 5) == 0){
